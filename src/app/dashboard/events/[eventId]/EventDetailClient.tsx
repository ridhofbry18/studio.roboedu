...
import { CalendarBlank, Plus, FileVideo, CheckCircle, Clock, PencilSimple, WarningCircle } from "@phosphor-icons/react";
import { addEventMeetingAndSubmission, updateEventSubmissionLink, reviewEventSubmission } from "@/app/actions/meetings";
import { toast } from "react-toastify";
import { Link as LinkIcon } from "@phosphor-icons/react";

export function EventDetailClient({ event, meetings, submissions, user }: { event: any, meetings: any[], submissions: any[], user: any }) {
  ...
  async function handleAddMeeting(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await addEventMeetingAndSubmission(event.id, driveLink);
      toast.success("Project Event berhasil diajukan ke SPV.");
  ...
  async function handleEditLink(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await updateEventSubmissionLink(selectedSubmissionId!, event.id, driveLink);
  ...
  async function handleReview(status: "approved" | "revision") {
    ...
    setLoading(true);
    try {
      await reviewEventSubmission(selectedSubmissionId!, event.id, status, reviewFeedback);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-border pb-4">
        <p className="text-sm text-foreground/70 flex-1">
          Project di dalam event dihitung otomatis. Anggota yang ditugaskan dapat mengajukan draf konten ke SPV.
        </p>
        <div className="flex items-center gap-3 shrink-0">
          {user.role === 'anggota' && (
            <button 
              onClick={() => setIsSubmitModalOpen(true)}
              className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-pink-400 to-rose-500 text-white rounded-xl font-bold shadow-md hover:shadow-pink-500/20 active:scale-95 transition-all text-sm"
            >
              <Plus weight="bold" /> Tambah Project (Materi)
            </button>
          )}
        </div>
      </div>

      {/* LIST KARTU PERTEMUAN/PROJECT */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {meetings.map(meeting => {
          ...
          return (
            <div key={meeting.id} className="p-5 bg-surface-variant rounded-[1.5rem] border border-border flex flex-col hover:border-pink-500/30 transition-colors shadow-sm">
        ...
        {meetings.length === 0 && (
          <div className="col-span-full p-12 text-center text-foreground/50 bg-surface-variant/50 rounded-[1.5rem] border border-dashed border-border">
            <CalendarBlank size={48} weight="duotone" className="mx-auto mb-3 opacity-30 text-pink-500" />
            <p className="font-semibold text-lg">Belum Ada Project/Materi</p>
            <p className="text-sm mt-1">Buat project pertama Anda dan lampirkan tautan GDrive publik.</p>
          </div>
        )}
      </div>
      ...
      {isSubmitModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-surface border border-border rounded-2xl w-full max-w-md p-6 shadow-2xl relative">
            ...
            <h3 className="font-bold text-xl mb-1">Project Event Baru</h3>
            <p className="text-xs text-foreground/60 mb-6">Ajukan materi/tugas baru untuk event ini. Judul project otomatis diurutkan.</p>
            <form onSubmit={handleAddMeeting} className="space-y-4">
              <div>
                ...
                <input name="driveLink" type="url" required className="w-full px-4 py-3 bg-surface-variant rounded-xl text-sm border border-border outline-none focus:border-pink-500 transition-colors" placeholder="https://drive.google.com/..." />
              ...
              <button disabled={loading} type="submit" className="w-full py-3 bg-gradient-to-r from-pink-400 to-rose-500 text-white font-bold rounded-xl hover:opacity-90 transition-opacity flex justify-center items-center gap-2">
          ...
          <div className="bg-surface border border-border rounded-2xl w-full max-w-md p-6 shadow-2xl relative">
            ...
            <form onSubmit={handleEditLink} className="space-y-4">
              <div>
                ...
                <input name="driveLink" type="url" defaultValue={selectedDriveLink} required className="w-full px-4 py-3 bg-surface-variant rounded-xl text-sm border border-border outline-none focus:border-pink-500 transition-colors" />
              </div>
              <button disabled={loading} type="submit" className="w-full py-3 bg-pink-500 text-white font-bold rounded-xl hover:opacity-90 transition-opacity">
      ...
      {isReviewModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-surface border border-border rounded-2xl w-full max-w-md p-6 shadow-2xl relative">
            ...
            <p className="text-xs text-foreground/60 mb-6">Tinjau tautan yang diajukan oleh anggota tim untuk event ini.</p>
            ...
            <div className="mb-6">
              ...
              <textarea 
                ...
                rows={3} 
                className="w-full px-4 py-3 bg-surface-variant rounded-xl text-sm border border-border outline-none focus:border-pink-500 transition-colors resize-none" 
